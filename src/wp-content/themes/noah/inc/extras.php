...
function noah_post_classes( $classes ) {
	...
	if ( is_post_type_archive( 'post' ) || is_date() || is_author() || is_home() || is_search() || is_post_type_archive( 'jetpack-portfolio' ) || pixelgrade_in_location( 'portfolio jetpack', $location )
...
if ( ! function_exists( 'noah_post_gallery' ) ) {
	...
	function noah_post_gallery( $value, $attr ) {
		...
		if ( ! empty( $attr['type'] ) && 'slideshow' == $attr['type'] ) {
            ...
            if ( ! empty( $attr['autostart'] ) && 'true' == $attr['autostart'] ) {
                $attr['columns'] = -1;
                unset( $attr['autostart'] );
            }
...
function noah_mce_before_init( $settings ) {
	...
	$style_formats = array(
		array( 'title' => esc_html__( 'Intro', 'noah' ), 'selector' => 'p', 'classes' => 'intro' ),
		array( 'title' => esc_html__( 'Dropcap', 'noah' ), 'inline' => 'span', 'classes' => 'dropcap' ),
		array( 'title' => esc_html__( 'Display', 'noah' ), 'inline' => 'span', 'classes' => 'h0' )
	);
 ...
 */
function noah_single_post_pages( $content ) {
...
add_filter( 'the_content', 'noah_single_post_pages', 16 );
...
function noah_tags_list( $content ) {
	...
	if ( 'post' == get_post_type() && is_single() ) {
		...
		if ( $tags_list && is_singular( 'post' ) ) {
			$tags_content .= '<div class="o-inline o-inline-xs tags h6"><div class="tags__title">' . esc_html__( 'Tags', 'noah' ) . sprintf( '</div>' . esc_html__( '%1$s', 'noah' ) . '</div>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
// This way the the author bio will appear before the related posts
...
function noah_add_no_js_hook_to_footer() {
	// This inline script can't be added using wp_add_inline_script() because it is not related to any registered script
	?>
...
add_action( 'wp_footer', 'noah_add_no_js_hook_to_footer', 10 );

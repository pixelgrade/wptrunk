...
if ( ! class_exists( 'Jetpack_Portfolio' ) && ! class_exists( 'Jetpack_Portfolio_Fallback' ) ) {

	class Jetpack_Portfolio_Fallback {
		...
		static function init() {
			...
			if ( ! $instance ) {
				$instance = new Jetpack_Portfolio_Fallback;
			}
		...
		function __construct() {
			...
			$setting = $this->get_option_and_ensure_autoload( self::OPTION_NAME, '0' );
		...
		function settings_api_init() {
			add_settings_field(
				self::OPTION_NAME,
				'<span class="cpt-options">' . __( 'Portfolio Projects', 'noah' ) . '</span>',
		...
		function setting_html() {
			if ( current_theme_supports( self::CUSTOM_POST_TYPE ) ) : ?>
				<p><?php printf( __( 'Your theme supports <strong>%s</strong>', 'noah' ), self::CUSTOM_POST_TYPE ); ?></p>
			<?php else : ?>
				<label for="<?php echo esc_attr( self::OPTION_NAME ); ?>">
					...
					<?php esc_html_e( 'Enable Portfolio Projects for this site.', 'noah' ); ?>
					<a target="_blank"
					   href="http://en.support.wordpress.com/portfolios/"><?php esc_html_e( 'Learn More', 'noah' ); ?></a>
				</label>
			<?php endif;
			if ( get_option( self::OPTION_NAME, '0' ) || current_theme_supports( self::CUSTOM_POST_TYPE ) ) :
				printf( '<p><label for="%1$s">%2$s</label></p>',
					esc_attr( self::OPTION_READING_SETTING ),
					sprintf( __( 'Portfolio pages display at most %1$s projects', 'noah' ),
		...
		function register_post_types() {
			...
			register_post_type( self::CUSTOM_POST_TYPE, array(
				'description'     => __( 'Portfolio Items', 'noah' ),
				'labels'          => array(
					'name'                  => esc_html__( 'Projects', 'noah' ),
					'singular_name'         => esc_html__( 'Project', 'noah' ),
					'menu_name'             => esc_html__( 'Portfolio', 'noah' ),
					'all_items'             => esc_html__( 'All Projects', 'noah' ),
					'add_new'               => esc_html__( 'Add New', 'noah' ),
					'add_new_item'          => esc_html__( 'Add New Project', 'noah' ),
					'edit_item'             => esc_html__( 'Edit Project', 'noah' ),
					'new_item'              => esc_html__( 'New Project', 'noah' ),
					'view_item'             => esc_html__( 'View Project', 'noah' ),
					'search_items'          => esc_html__( 'Search Projects', 'noah' ),
					'not_found'             => esc_html__( 'No Projects found', 'noah' ),
					'not_found_in_trash'    => esc_html__( 'No Projects found in Trash', 'noah' ),
					'filter_items_list'     => esc_html__( 'Filter projects list', 'noah' ),
					'items_list_navigation' => esc_html__( 'Project list navigation', 'noah' ),
					'items_list'            => esc_html__( 'Projects list', 'noah' ),
				),
				'supports'        => array(
					'title',
					'editor',
			...
			register_taxonomy( self::CUSTOM_TAXONOMY_TYPE, self::CUSTOM_POST_TYPE, array(
				'hierarchical'      => true,
				'labels'            => array(
					'name'                  => esc_html__( 'Project Types', 'noah' ),
					'singular_name'         => esc_html__( 'Project Type', 'noah' ),
					'menu_name'             => esc_html__( 'Project Types', 'noah' ),
					'all_items'             => esc_html__( 'All Project Types', 'noah' ),
					'edit_item'             => esc_html__( 'Edit Project Type', 'noah' ),
					'view_item'             => esc_html__( 'View Project Type', 'noah' ),
					'update_item'           => esc_html__( 'Update Project Type', 'noah' ),
					'add_new_item'          => esc_html__( 'Add New Project Type', 'noah' ),
					'new_item_name'         => esc_html__( 'New Project Type Name', 'noah' ),
					'parent_item'           => esc_html__( 'Parent Project Type', 'noah' ),
					'parent_item_colon'     => esc_html__( 'Parent Project Type:', 'noah' ),
					'search_items'          => esc_html__( 'Search Project Types', 'noah' ),
					'items_list_navigation' => esc_html__( 'Project type list navigation', 'noah' ),
					'items_list'            => esc_html__( 'Project type list', 'noah' ),
				),
				'public'            => true,
				'show_ui'           => true,
			...
			register_taxonomy( self::CUSTOM_TAXONOMY_TAG, self::CUSTOM_POST_TYPE, array(
				'hierarchical'      => false,
				'labels'            => array(
					'name'                       => esc_html__( 'Project Tags', 'noah' ),
					'singular_name'              => esc_html__( 'Project Tag', 'noah' ),
					'menu_name'                  => esc_html__( 'Project Tags', 'noah' ),
					'all_items'                  => esc_html__( 'All Project Tags', 'noah' ),
					'edit_item'                  => esc_html__( 'Edit Project Tag', 'noah' ),
					'view_item'                  => esc_html__( 'View Project Tag', 'noah' ),
					'update_item'                => esc_html__( 'Update Project Tag', 'noah' ),
					'add_new_item'               => esc_html__( 'Add New Project Tag', 'noah' ),
					'new_item_name'              => esc_html__( 'New Project Tag Name', 'noah' ),
					'search_items'               => esc_html__( 'Search Project Tags', 'noah' ),
					'popular_items'              => esc_html__( 'Popular Project Tags', 'noah' ),
					'separate_items_with_commas' => esc_html__( 'Separate tags with commas', 'noah' ),
					'add_or_remove_items'        => esc_html__( 'Add or remove tags', 'noah' ),
					'choose_from_most_used'      => esc_html__( 'Choose from the most used tags', 'noah' ),
					'not_found'                  => esc_html__( 'No tags found.', 'noah' ),
					'items_list_navigation'      => esc_html__( 'Project tag list navigation', 'noah' ),
					'items_list'                 => esc_html__( 'Project tag list', 'noah' ),
				),
				'public'            => true,
				'show_ui'           => true,
				'show_in_nav_menus' => true,
		...
		function updated_messages( $messages ) {
			...
			$messages[ self::CUSTOM_POST_TYPE ] = array(
				...
				1  => sprintf( __( 'Project updated. <a href="%s">View item</a>', 'noah' ), esc_url( get_permalink( $post->ID ) ) ),
				2  => esc_html__( 'Custom field updated.', 'noah' ),
				3  => esc_html__( 'Custom field deleted.', 'noah' ),
				4  => esc_html__( 'Project updated.', 'noah' ),
				/* translators: %s: date and time of the revision */
				5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Project restored to revision from %s', 'noah' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				6  => sprintf( __( 'Project published. <a href="%s">View project</a>', 'noah' ), esc_url( get_permalink( $post->ID ) ) ),
				7  => esc_html__( 'Project saved.', 'noah' ),
				8  => sprintf( __( 'Project submitted. <a target="_blank" href="%s">Preview project</a>', 'noah' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
				9  => sprintf( __( 'Project scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview project</a>', 'noah' ),
					// translators: Publish box date format, see http://php.net/date
					date_i18n( __( 'M j, Y @ G:i', 'noah' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post->ID ) ) ),
				10 => sprintf( __( 'Project item draft updated. <a target="_blank" href="%s">Preview project</a>', 'noah' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
			);
		...
		function customize_register( $wp_customize ) {
			...
			$wp_customize->add_section( 'jetpack_portfolio', array(
				'title'          => esc_html__( 'Portfolio', 'noah' ),
			...
			if ( isset( $options[0]['title'] ) && true === $options[0]['title'] ) {
				$wp_customize->add_setting( 'jetpack_portfolio_title', array(
					'default'              => esc_html__( 'Projects', 'noah' ),
					'type'                 => 'option',
				...
				$wp_customize->add_control( 'jetpack_portfolio_title', array(
					...
					'label'   => esc_html__( 'Portfolio Archive Title', 'noah' ),
					'type'    => 'text',
				) );
			}
			...
			if ( isset( $options[0]['content'] ) && true === $options[0]['content'] ) {
				...
				$wp_customize->add_control( 'jetpack_portfolio_content', array(
					'section' => 'jetpack_portfolio',
					'label'   => esc_html__( 'Portfolio Archive Content', 'noah' ),
					'type'    => 'textarea',
				) );
			}
			...
			if ( isset( $options[0]['featured-image'] ) && true === $options[0]['featured-image'] ) {
				...
				$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'jetpack_portfolio_featured_image', array(
					'section' => 'jetpack_portfolio',
					'label'   => esc_html__( 'Portfolio Archive Featured Image', 'noah' ),
				) ) );
			}
		}
		...
		static function portfolio_shortcode_html( $atts ) {
			...
			if ( $query->have_posts() ) {
				...
				<p>
					<em><?php _e( 'Your Portfolio Archive currently has no entries. You can start creating them on your dashboard.', 'noah' ); ?>
				</p></em>
				<?php
			}
			$html = ob_get_clean();
		...
		static function get_project_type( $post_id ) {
			...
			$html  = '<div class="project-types"><span>' . __( 'Types', 'noah' ) . ':</span>';
			$types = array();
		...
		static function get_project_tags( $post_id ) {
			...
			$html = '<div class="project-tags"><span>' . __( 'Tags', 'noah' ) . ':</span>';
			$tags = array();
		...
		static function get_project_author() {
			...
			$html .= sprintf( __( '<span>Author:</span> <a href="%1$s">%2$s</a>', 'noah' ),
	...
	add_action( 'init', array( 'Jetpack_Portfolio_Fallback', 'init' ) );

	// Check on plugin activation if theme supports CPT
	register_activation_hook( __FILE__, array( 'Jetpack_Portfolio_Fallback', 'activation_post_type_support' ) );
	add_action( 'jetpack_activate_module_custom-content-types', array(
		'Jetpack_Portfolio_Fallback',

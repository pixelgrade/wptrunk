...
// If Jetpack is not active load the code for the portfolio custom post type from the theme
function noah_load_jetpack_custom_post_types() {
	...
	if ( ! post_type_exists( 'jetpack-portfolio' ) ) {
		require_once 'jetpack/custom-content-types.php';
		require_once 'jetpack/portfolios.php';
	}
}
add_action( 'after_setup_theme', 'noah_load_jetpack_custom_post_types', 20 );
...
function noah_remove_sharing_on_portfolio_archives() {
	if ( 'jetpack-portfolio' === get_post_type() && ! is_singular( 'jetpack-portfolio' ) ) {
		remove_filter( 'the_content', 'sharing_display',19 );
		remove_filter( 'the_excerpt', 'sharing_display',19 );
	}
}
add_action( 'wp_head', 'noah_remove_sharing_on_portfolio_archives', 10 );
	...
	function enqueue_assets( $output ) {
		...
		if ( $this->first_run ) {
			...
			$localize_strings = array(
				...
				'camera'               => esc_html__( 'Camera', 'noah' ),
				'aperture'             => esc_html__( 'Aperture', 'noah' ),
				'shutter_speed'        => esc_html__( 'Shutter Speed', 'noah' ),
				'focal_length'         => esc_html__( 'Focal Length', 'noah' ),
			);

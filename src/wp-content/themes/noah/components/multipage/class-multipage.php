...
class Pixelgrade_Multipage {
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'noah_multipage-scripts', pixelgrade_get_theme_file_uri( 'components/multipage/js/jquery.bully.js' ), array( 'jquery' ), $this->_assets_version, true );

		// See if we need to enqueue something for multipages
		if ( is_page() && pixelgrade_multipage_has_children() ) {
			wp_enqueue_script( 'noah_multipage-scripts' );
		}
	}
	...
	function subpages_admin_bar_edit_links_backend( $wp_admin_bar ) {
		...
		if ( $is_edit_page && isset( $post->post_parent ) && ! empty( $post->post_parent ) ) {
			...
			$wp_admin_bar->add_node( array(
				'id'    => 'edit_parent',
				'title' => esc_html__( 'Edit Parent', 'noah' ),
			...
			if ( isset($siblings[ $current_pos - 1 ] ) ){
				...
				$wp_admin_bar->add_node( array(
					'id'    => 'edit_prev_child',
					'title' => esc_html__( 'Edit Prev Child', 'noah' ),
			...
			if ( isset($siblings[ $current_pos + 1 ] ) ) {
				...
				$wp_admin_bar->add_node( array(
					'id'    => 'edit_next_child',
					'title' => esc_html__( 'Edit Next Child', 'noah' ),
		...
		if ( $is_edit_page ) {
			...
			if ( ! empty( $kids ) ) {
				...
				$args = array(
					'id'    => 'edit_children',
					'title' => esc_html__( 'Edit Children', 'noah' ),
					'href'  => '#',
				...
				foreach ( $kids as $kid ) {
					$kid_args = array(
						...
						'title' => esc_html__( 'Edit', 'noah' ) . ': ' . $kid->post_title,

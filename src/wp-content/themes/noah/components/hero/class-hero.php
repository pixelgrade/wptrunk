...
class Pixelgrade_Hero {
			...
			'hero_area_content__page'     => array(
				...
				'fields'     => array(
					...
					array(
						...
						'value'   => esc_attr__( 'Add more than one image to the <strong>Hero Area » Background</strong> to enable this section. ', 'noah' ),
						'type'    => 'title',
					),
					array(
			...
			'hero_area_map__page' => array(
				...
				'fields'     => array(
					...
					array(
						...
						'desc' => __( 'Paste here the <strong>Share Link</strong>URL you have copied from <a href="https://www.google.com/maps" target="_blank">Google Maps</a>. Do not use the embed code or a short URL.', 'noah' ),
						'id'   => '_hero_map_url',
						'type' => 'textarea_small',
						'std'  => '',
					),
	...
	public function admin_init() {
		...
		wp_register_style( 'noah_hero-admin-style', pixelgrade_get_theme_file_uri( 'components/hero/css/admin.css' ), array(), $this->_assets_version );
		wp_register_script( 'noah_hero-admin-scripts', pixelgrade_get_theme_file_uri( 'components/hero/js/admin.js' ), array(), $this->_assets_version );
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( 'edit.php' != $hook ) {
			wp_enqueue_style( 'noah_hero-admin-style');
			wp_enqueue_script( 'noah_hero-admin-scripts' );
	...
	public function _add_video_url_field_to_attachments( $form_fields, $post ) {
		...
		if ( ! isset( $form_fields["video_url"] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_video_url' ) {
			$form_fields["video_url"] = array(
				...
				"helps" => esc_attr__( "<p class='desc'>Attach a video to this image <span class='small'>(YouTube or Vimeo)</span>.</p>", 'noah' ),
			);
		}
		...
		if ( ! isset( $form_fields["custom_image_url"] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_image_url' ) {
			$form_fields["custom_image_url"] = array(
				...
				"helps" => esc_attr__( "<p class='desc'>Link this image to a custom url.</p>", 'noah' ),
			);
		}
		...
		if ( ! isset( $form_fields["video_autoplay"] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_video_url' ) {
			...
			$form_fields["video_autoplay"] = array(
				...
				"input" => "html",
				"html"  => '<input' . $checked . ' type="checkbox" name="attachments[' . $post->ID . '][video_autoplay]" id="attachments[' . $post->ID . '][video_autoplay]" /><label for="attachments[' . $post->ID . '][video_autoplay]">' . esc_html__( 'Enable Video Autoplay?', 'noah' ) . '</label>'
			);
		}
		...
		if ( ! isset( $form_fields["external_url"] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'external' ) {
			$form_fields["external_url"] = array(
				...
				"helps" => esc_attr__( "<p class='desc'>Set this image to link to an external website.</p>", 'noah' ),
			);
		}
